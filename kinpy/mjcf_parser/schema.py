...
_ARRAY_DTYPE_MAP = {"int": int, "float": float, "string": str}
...
def _parse_element(element_xml):
    ...
    if common_keys:
        raise RuntimeError(
            ...
            "this library. Please file a bug report. Thank you.".format(
                name, sorted(common_keys)
            )
        )
...
def _attachment_frame_spec(is_world_attachment):
    ...
    if is_world_attachment:
        freejoint_spec = (
            MUJOCO.children["worldbody"].children["body"].children["freejoint"]
        )
